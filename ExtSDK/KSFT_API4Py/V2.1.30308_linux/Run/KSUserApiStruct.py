...
class CThostFtdcQryCommRateModelField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('CommModelID', u'手续费率模板代码'),('BrokerID', u'经纪公司代码')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CTKSConditionalOrderRemove:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InvestorID', u'投资者代码'),('BrokerID', u'营业部代码'),('ConditionalOrderID', u'条件单编号')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcSyncingInstrumentTradingRightField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InstrumentID', u'合约代码'),('InvestorID', u'投资者代码'),('BrokerID', u'经纪公司代码'),('InvestorRange', u'投资者范围'),('TradingRight', u'交易权限')]])
    def getval(self, n):
...
class CThostFtdcQryTradingCodeField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InvestorID', u'投资者代码'),('ExchangeID', u'交易所代码'),('BrokerID', u'经纪公司代码'),('ClientIDType', u'交易编码类型'),('ClientID', u'客户代码')]])
    def getval(self, n):
...
class CTKSCOSStatusField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('ConditionalOrderStatus', u'条件单状态'),('CombOffsetFlag', u'开平标志'),('UserID', u'操作员'),('LimitPrice', u'价格'),('Direction', u'买卖方向'),('ActiveTime', u'激活时间'),('Memo', u'备注'),('ClientID', u'客户代码'),('VolumeTotal', u'剩余数量'),('VolumeTraded', u'今成交数量'),('OrderStatus', u'报单状态'),('OrderSysID', u'报单编号'),('CurrencyID', u'货币代码'),('InstrumentID', u'合约代码'),('ExchangeID', u'交易所代码'),('CancelUserID', u'撤销用户'),('TradePrice', u'成交价格'),('TradingDay', u'交易日'),('BrokerID', u'营业部代码'),('InsertTime', u'委托时间'),('CombHedgeFlag', u'投机套保标志'),('CancelTime', u'撤销时间'),('OrderLocalID', u'本地报单编号'),('BusinessUnit', u'业务单元'),('SequenceNo', u'序号'),('InvestorID', u'投资者代码'),('ConditionalOrderID', u'条件单编号'),('VolumeTotalOriginal', u'数量')]])
    def getval(self, n):
...
class CThostFtdcSyncingInvestorField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('CommModelID', u'手续费率模板代码'),('InvestorName', u'投资者名称'),('Mobile', u'手机'),('IdentifiedCardNo', u'证件号码'),('Telephone', u'联系电话'),('InvestorID', u'投资者代码'),('BrokerID', u'经纪公司代码'),('Address', u'通讯地址'),('InvestorGroupID', u'投资者分组代码'),('OpenDate', u'开户日期'),('IsActive', u'是否活跃'),('IdentifiedCardType', u'证件类型')]])
    def getval(self, n):
...
class CThostFtdcSyncingTradingCodeField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('ExchangeID', u'交易所代码'),('ClientID', u'客户代码'),('InvestorID', u'投资者代码'),('BrokerID', u'经纪公司代码'),('ClientIDType', u'交易编码类型'),('IsActive', u'是否活跃')]])
    def getval(self, n):
        if n in ['ClientIDType']:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
class CTKSCOSAskSelectField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('SequenceNo', u'序号'),('Memo', u'备注'),('UserID', u'操作员'),('InvestorID', u'投资者代码'),('BrokerID', u'营业部代码'),('ConditionalOrderID', u'条件单编号'),('SelectType', u'选择方式')]])
    def getval(self, n):
        if n in ['SelectType']:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CTKSConditionalOrderSelect:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InvestorID', u'投资者代码'),('BrokerID', u'营业部代码'),('ConditionalOrderID', u'条件单编号'),('SelectResult', u'选择结果')]])
    def getval(self, n):
...
class CTKSConditionalOrderStateAlter:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InvestorID', u'投资者代码'),('ConditionalOrderStateAlter', u'暂停或激活操作标志'),('BrokerID', u'营业部代码'),('ConditionalOrderID', u'条件单编号')]])
    def getval(self, n):
...
class CThostFtdcQryInstrumentMarginRateField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InstrumentID', u'合约代码'),('InvestorID', u'投资者代码'),('BrokerID', u'经纪公司代码'),('HedgeFlag', u'投机套保标志')]])
    def getval(self, n):
        if n in ['HedgeFlag']:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CTKSProfitAndLossOrderQuery:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('ExchangeID', u'交易所代码'),('OrderLocalID', u'本地报单编号'),('BusinessUnit', u'业务单元'),('InvestorID', u'投资者代码'),('BrokerID', u'营业部代码'),('ProfitAndLossOrderID', u'止损止盈单号')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CTKSProfitAndLossOrderModify:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('TakeProfitPrice', u'止盈价'),('TriggeredTimes', u'触发次数'),('SpringType', u'条件单触发价格类型'),('InvestorID', u'投资者代码'),('BrokerID', u'营业部代码'),('FiguresPrice', u'平仓反向价位数（平仓方式=1或2时有效）'),('ProfitAndLossOrderID', u'止损止盈单号'),('CloseMode', u'平仓方式'),('OffsetValue', u'生成盈损价的方式'),('StopLossPrice', u'止损价'),('FloatLimitPrice', u'浮动止损价')]])
    def getval(self, n):
        if n in ['SpringType', 'CloseMode', 'OffsetValue']:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcCommRateModelField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('CommModelID', u'手续费率模板代码'),('BrokerID', u'经纪公司代码'),('CommModelName', u'模板名称')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcUserRightsAssignField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('UserID', u'用户代码'),('DRIdentityID', u'交易中心代码'),('BrokerID', u'应用单元代码')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostOrderKeyField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('FrontID', u'前置编号'),('SessionID', u'会话编号'),('ExchangeID', u'交易所代码'),('OrderSysID', u'报单编号'),('OrderRef', u'报单引用')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcBulkCancelOrderField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InvestorID', u'投资者代码'),('BrokerID', u'经纪公司代码'),('nCount', u'报单个数'),('UserID', u'用户代码'),('OrderType', u'报单类型')]])
    def getval(self, n):
        if n in ['OrderType']:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcBrokerUserRightAssignField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('DRIdentityID', u'交易中心代码'),('BrokerID', u'应用单元代码'),('Tradeable', u'能否交易')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
class CTKSConditionalOrderOperResultField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('ConditionalOrderStatus', u'条件单状态'),('CombOffsetFlag', u'开平标志'),('UserID', u'撤销用户'),('LimitPrice', u'价格'),('Direction', u'买卖方向'),('ActiveTime', u'有效时间'),('TriggeredTimes', u'触发次数'),('Memo', u'备注'),('ClientID', u'客户代码'),('OrderPriceType', u'价格类别'),('OrderStatus', u'报单状态'),('InstrumentID', u'合约代码'),('ExchangeID', u'交易所代码'),('OrderType', u'条件单类型'),('TradingDay', u'交易日'),('BrokerID', u'营业部代码'),('CombHedgeFlag', u'投机套保标志'),('CancelTime', u'撤销时间'),('OrderLocalID', u'本地报单编号'),('ErrorMsg', u'错误信息'),('InvestorID', u'投资者代码'),('InActiveTime', u'失效时间'),('ConditionalOrderID', u'条件单编号'),('VolumeTotalOriginal', u'数量')]])
    def getval(self, n):
...
class CTKSProfitAndLossOrderRemoveField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InvestorID', u'投资者代码'),('BrokerID', u'营业部代码'),('ProfitAndLossOrderID', u'止损止盈单号')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcQryProductField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('ProductID', u'产品代码')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcAccountregisterField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('BankAccount', u'银行帐号'),('CustType', u'客户类型'),('CustomerName', u'客户姓名'),('CurrencyID', u'币种代码'),('BrokerBranchID', u'期货公司分支机构编码'),('OutDate', u'解约日期'),('IdentifiedCardNo', u'证件号码'),('BankBranchID', u'银行分支机构编码'),('RegDate', u'签约日期'),('BrokerID', u'期货公司编码'),('BankID', u'银行编码'),('TID', u'交易ID'),('OpenOrDestroy', u'开销户类别'),('IdCardType', u'证件类型'),('TradeDay', u'交易日期'),('BankAccType', u'银行帐号类型'),('AccountID', u'投资者帐号')]])
    def getval(self, n):
...
class CThostFtdcDRTransferField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('DestBrokerID', u'目标易用单元代码'),('DestDRIdentityID', u'目标交易中心代码'),('OrigDRIdentityID', u'原交易中心代码'),('OrigBrokerID', u'原应用单元代码')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcTransferSerialField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('BankNewAccount', u'新银行帐号'),('BrokerBranchID', u'期商分支机构代码'),('TradeTime', u'交易时间'),('OperatorCode', u'操作员'),('AccountID', u'投资者帐号'),('BankAccount', u'银行帐号'),('TradeCode', u'交易代码'),('BankBranchID', u'银行分支机构编码'),('SessionID', u'会话编号'),('BankID', u'银行编码'),('PlateSerial', u'平台流水号'),('FutureAccType', u'期货公司帐号类型'),('ErrorID', u'错误代码'),('BankSerial', u'银行流水号'),('IdentifiedCardNo', u'证件号码'),('TradingDay', u'交易日期'),('BrokerID', u'期货公司编码'),('IdCardType', u'证件类型'),('TradeDate', u'交易发起方日期'),('CurrencyID', u'币种代码'),('BrokerFee', u'应收期货公司费用'),('BankAccType', u'银行帐号类型'),('FutureSerial', u'期货公司流水号'),('InvestorID', u'投资者代码'),('ErrorMsg', u'错误信息'),('CustFee', u'应收客户费用'),('TradeAmount', u'交易金额'),('AvailabilityFlag', u'有效标志')]])
    def getval(self, n):
...
class CThostFtdcTraderField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('ExchangeID', u'交易所代码'),('ParticipantID', u'会员代码'),('TraderID', u'交易所交易员代码'),('BrokerID', u'经纪公司代码'),('InstallCount', u'安装数量'),('Password', u'密码')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcLoginInfoField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('CZCETime', u'郑商所时间'),('MacAddress', u'Mac地址'),('SHFETime', u'上期所时间'),('MaxOrderRef', u'最大报单引用'),('UserProductInfo', u'用户端产品信息'),('InterfaceProductInfo', u'接口端产品信息'),('UserID', u'用户代码'),('LoginDate', u'登录日期'),('SystemName', u'系统名称'),('SessionID', u'会话编号'),('BrokerID', u'经纪公司代码'),('OneTimePassword', u'动态密码'),('FFEXTime', u'中金所时间'),('FrontID', u'前置编号'),('DCETime', u'大商所时间'),('Password', u'密码'),('IPAddress', u'IP地址'),('LoginTime', u'登录时间'),('ProtocolInfo', u'协议信息')]])
    def getval(self, n):
        if n in []:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
...
class CThostFtdcEWarrantOffsetField:
    ...
    def __str__(self):  return "<%s>" % ",".join(["%s:%s" % (y.encode("utf-8"), self.getval(x)) for x,y in [('InstrumentID', u'合约代码'),('ExchangeID', u'交易所代码'),('InvestorID', u'投资者代码'),('HedgeFlag', u'投机套保标志'),('Direction', u'买卖方向'),('Volume', u'数量'),('TradingDay', u'交易日期'),('BrokerID', u'经纪公司代码')]])
    def getval(self, n):
        if n in ['HedgeFlag', 'Direction']:
            return self.vcmap[n]["'%s'" % getattr(self, n)].encode("utf-8")
        else: return getattr(self, n)
